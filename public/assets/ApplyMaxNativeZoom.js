...
L.BingLayer.include({
  applyMaxNativeZoom: function (latlng) {
    ...
    var request = this._makeApiUrl(
      "Imagery/BasicMetadata",
      L.Util.template("{imagerySet}/{centerPoint}", {
    ...
    this._findVintage(request, options.maxZoom + zoomOffset, function (zoom) {
      ...
      if (newLatlng.distanceTo(latlng) > validityRadius) {
        this.applyMaxNativeZoom(newLatlng);
        return;
      }
      zoom -= zoomOffset;
      ...
      if (
        (zoom < oldValue && zoom < mapZoom) ||
        (zoom > oldValue && mapZoom > oldValue)
      ) {
        this._resetView();
      }
